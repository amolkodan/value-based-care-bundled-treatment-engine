...
| `vbc.episode_rule` | Rules with `rule_role` **INDEX**, **INCLUSION**, or **EXCLUSION** and `code_system` ICD10 / CPT / HCPCS / NDC |
...
| `vbc.claim_episode_assignment` | Many-to-many link from medical claim or pharmacy line to an instance |
...
- `episode_rule.csv` — exactly one of `code_set_id` or `code_value` per row
